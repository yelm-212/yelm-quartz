...
문서 구조와 진행 순서는 [트러블슈팅 문서 구조](#트러블슈팅-문서-구조)를 따른다.
...
2. 실제 공개 사례 중 다음과 관련된 사례

* File Descriptor 고갈
* Socket 또는 File 자원 누수
* Limit 설정 문제
...
4. 사례의 조사 과정 재구성
5. 실제 원인 확인
6. 내 가설과 실제 원인의 차이 분석
7. 이번 주 학습한 CS 개념으로 다시 설명
```

### 트러블슈팅 문서 구조

모든 주차의 트러블슈팅 문서는 다음 구조를 공통으로 사용한다. 주차별로 달라지는 것은 사례 후보와 2~3단계에서 사용하는 구간 구분이며, 절의 구성과 순서는 동일하게 유지한다.

| 절 | 기록할 내용 |
| --- | --- |
| 사례 선정 기준 | 어떤 조건을 만족하는 사례를 선택했는지 |
| 사례 출처와 배경 | 조직, 시스템의 역할, 발생 시점, 원문 링크 |
| 처음 관찰된 증상 | 사용자 영향과 최초 로그·메트릭만. 여기까지 읽고 먼저 추론한다 |
| 1. 문제 상황 파악 | 영향 범위, 시작 시점, 재현 조건, 최근 변경 사항 |
| 2. 원인 가설 설정 | 구간별 가설과 근거, 예상되는 관찰 결과 |
| 3. 확인할 로그, 메트릭, 명령어 정의 | 구간별로 무엇을 어떤 기준으로 확인할지 |
| 4. 조사 과정 | 사례의 조사 순서를 시간순으로 재구성 |
| 5. 실제 원인 확인 | 직접 원인, 근본 원인, 이를 뒷받침한 증거 |
| 6. 가설과 실제 원인 비교 | 판정, 실제 결과와의 차이, 놓친 단서 |
| 해결 방법과 재발 방지 | 즉시 조치와 장기 개선을 구분 |
| 개인 경험과의 연결 | 자연스럽게 연결될 때만 작성 |
| 배운 점 | 같은 증상에서 재사용할 판단 기준 |

기존 5단계에서 **4. 조사 과정**을 추가해 6단계로 진행한다. 실제 원인을 바로 확인하지 않고, 사례가 어떤 순서로 무엇을 관찰해 원인에 도달했는지를 먼저 재구성한다. 그래야 내 가설이 틀린 지점뿐 아니라 조사 순서가 달랐던 지점까지 비교할 수 있다.

각 단계는 다음을 지킨다.

- 2단계 가설표에는 어느 구간을 의심하는지 함께 적는다.
- 3단계는 구간별로 나눠 적고, 운영 환경에서 실행할 때의 권한과 부하도 확인한다.
- 5단계는 직접 원인과 근본 원인을 구분한다.
- 해결 방법은 즉시 조치와 장기 개선을 나누고, 개선 항목마다 검증 방법을 적는다.
